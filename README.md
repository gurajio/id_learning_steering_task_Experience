...
2. 参加者IDを入力します。
3. 「実験開始」ボタンを押します。
4. 実験専用ページへ切り替わります。ページ上部に現在条件、試行数、中止/再開、1分休憩、終了ボタンが表示されます。
5. 開始円から終了円まで、表示された通路に沿って操作します。
6. 試行が終わると次の開始円が表示されます。次の開始円を押した瞬間から次試行のMT計測が始まります。
7. 実験後、CSVまたはJSONを保存します。途中で終了した場合も、記録済みの結果を保存できます。
...
- 実験ページの中止/再開、1分休憩、終了ボタンのクリックイベントを登録します。
...
右側の実験キャンバスは `css/styles.css` で `1100 / 740` の比率に固定しています。条件リストが長くなった場合は左側だけがスクロールし、右側の表示比率は変わりません。
